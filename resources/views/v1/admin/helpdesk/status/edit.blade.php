...
<div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary">Status: {{ $status->code }} - {{ $status->name }}</h6>
    </div>
</div>
<form action="v1/admin/helpdesk/status/edit/{{ $status->id }}" method="post">
  @csrf
  <div class="form-row">
    ...
    <div class="col-md-2 mb-3">
      <label>Code</label>
      <input type="text" class="form-control" name="code" placeholder="Code..." value="{{$status->code}}">
    </div>
    <div class="col-md-4 mb-3">
      <label>Label</label>
      <input type="text" class="form-control" name="label" placeholder="Label..." value="{{$status->label}}">
    </div>
    <div class="col-md-4 mb-3">
      <label>Note</label>
      <input type="text" class="form-control" name="note" placeholder="Note" value="{{$status->note}}">
    </div>
